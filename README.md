...
1. **注册 / 登录**
   ...
   - **修改邮箱**：在右上角「设置」里发 6 位验证码到新邮箱，校验通过并
     输入当前密码后即可换登录邮箱。旧邮箱进入「曾用邮箱」列表；之后只
     能用当前邮箱登录，但只要当前邮箱**或任意一个曾用邮箱**还在 allowlist
     里就允许登录 —— 给已离开但仍想以新邮箱继续访问的成员留一条退路。
   - 修改邮箱会让所有已登录的设备失效，新邮箱必须重新登录一次以确认信箱
     由你掌控。
2. **领取写入令牌**
   - 进来之后先点左下角「获取 / 轮换令牌」，拿到一枚随机 32 字节的令牌。还没领令牌时，登录后会自动弹出这个对话框提醒你。
...
| `user_email_history` | 是（账号粒度）          | 用户曾经使用过的登录邮箱（用于扩展白名单门禁）  |
...
白名单同时是登录门禁：登录时只要账号的**当前邮箱或任意曾用邮箱**还在
名单里就放行。把一个邮箱从白名单里删掉，下次登录前还在名单里的曾用
邮箱仍然有效；要彻底踢人，需要把该账号关联过的所有邮箱都删掉。
